...
export default function ExecutiveDashboard() {
    ...
    const fetchData = async () => {
      try {
        ...
        setData({
            ...
            stats, peakRainDay, memberCount: rainKeys.length || 1, ensembleModelUsed,
            tmdInfo: tmdRes.info,
  ...
  const BoLuangMap = () => {
    ...
    // 🌟 State จดจำว่าผู้ใช้แตะ/ชี้ ที่โซนและหมุดไหนล่าสุด
    const [activeVillageIndex, setActiveVillageIndex] = useState<number | null>(null);
    const [activePin, setActivePin] = useState<string | null>(null);

    // ฟังก์ชันล้างค่า Active (เมื่อคลิกที่พื้นหลังแผนที่)
    ...
    const renderGeoJsonMap = () => {
        ...
        const villagePaths = geoVillages && geoVillages.features ? geoVillages.features.map((f: any, i: number) => {
            ...
            const activeColor = isAlert ? '#e11d48' : baseColor;
            
            // ตรวจสอบสถานะว่าโซนนี้โดนแตะ/ชี้ อยู่หรือไม่
            const isHovered = activeVillageIndex === i;
            
            return (
               <g key={`village-${i}`}
                  onMouseEnter={() => setActiveVillageIndex(i)}
                  onClick={(e) => { e.stopPropagation(); setActiveVillageIndex(i); setActivePin(null); }}
                  onTouchStart={(e) => { e.stopPropagation(); setActiveVillageIndex(i); setActivePin(null); }}
               >
                 <path d={generatePathD(f)}
                       fill={activeColor}
                       stroke={isHovered ? '#ffffff' : activeColor}
                       strokeWidth={isHovered ? "2.5" : "1.5"}
                       className={`transition-all duration-300 cursor-pointer ${isHovered ? 'opacity-70 shadow-lg' : 'opacity-20 hover:opacity-40'}`} />
               </g>
            );
        }) : null;
        ...
        // 📍 หมุดพร้อมระบบ React State เพื่อรองรับทัชบนมือถือ
        const renderVillageNode = (xOffset: number, yOffset: number, name: string, boxWidth: number = 75) => {
            const isActive = activePin === name;
            return (
                <g transform={`translate(${eocProj.x + xOffset}, ${eocProj.y + yOffset})`} 
                   className="cursor-pointer"
                   onMouseEnter={() => setActivePin(name)}
                   onMouseLeave={() => setActivePin(null)}
                   onClick={(e) => { e.stopPropagation(); setActivePin(isActive ? null : name); setActiveVillageIndex(null); }}
                   onTouchStart={(e) => { e.stopPropagation(); setActivePin(isActive ? null : name); setActiveVillageIndex(null); }}
                >
                   <circle cx="0" cy="0" r="15" fill="transparent" />
                   <circle cx="0" cy="0" r={isActive ? "5" : "3.5"} fill={isActive ? "#38bdf8" : "#94a3b8"} className="transition-all duration-300 shadow-lg" />
                   
                   <g className={`transition-all duration-300 transform ${isActive ? 'opacity-100 translate-x-1' : 'opacity-0'} pointer-events-none`}>
        ...
        return (
            <g>
               ...
               {isAlert && <circle cx={eocProj.x} cy={eocProj.y} r="80" fill="url(#alertHeat)" className="pointer-events-none" />}
               
               {/* 📍 หมุดศูนย์บัญชาการ EOC (บ่อหลวง) - แสดงค้างไว้เสมอ */}
               <g transform={`translate(${eocProj.x}, ${eocProj.y})`} className="pointer-events-none">
               ...
               {/* จุดหมู่บ้านอื่นๆ (ใช้ State ทำให้กดติดทัชบนมือถือได้) */}
               {renderVillageNode(-35, -45, "บ้านแม่หืด", 75)}
               {renderVillageNode(25, -25, "บ้านขุน", 60)}
               {renderVillageNode(-15, 45, "บ้านกิ่วป่าซี", 80)}
               {renderVillageNode(-55, 15, "บ้านเตียนอาง", 85)}
            </g>
        );
    };
    ...
    return (
      <div className="relative w-full h-full min-h-[350px] rounded-2xl bg-[#020617] border border-slate-800 shadow-inner overflow-hidden flex items-center justify-center">
        ...
        <svg viewBox="0 0 500 350" className="w-full h-full relative z-10">
          ...
          {/* 🌟 Background Rect สำหรับเคลียร์การเลือกบนมือถือ */}
          <rect x="0" y="0" width="500" height="350" fill="transparent" onClick={clearSelection} onTouchStart={clearSelection} />

          {renderGeoJsonMap()}

          <path d="M 10 30 L 10 10 L 30 10" fill="none" stroke="#334155" strokeWidth="2" className="pointer-events-none"/>
          <path d="M 490 30 L 490 10 L 470 10" fill="none" stroke="#334155" strokeWidth="2" className="pointer-events-none"/>
          <path d="M 10 320 L 10 340 L 30 340" fill="none" stroke="#334155" strokeWidth="2" className="pointer-events-none"/>
          <path d="M 490 320 L 490 340 L 470 340" fill="none" stroke="#334155" strokeWidth="2" className="pointer-events-none"/>
          
          <text x="15" y="335" fill="#475569" fontSize="8" fontFamily="monospace" className="pointer-events-none">LAT: 18.1633° N</text>
          <text x="415" y="335" fill="#475569" fontSize="8" fontFamily="monospace" className="pointer-events-none">LNG: 98.3744° E</text>
        </svg>

        <div className="absolute top-4 left-4 bg-slate-950/80 border border-slate-800 backdrop-blur-md p-2 rounded-lg shadow-[0_4px_20px_rgba(0,0,0,0.5)] pointer-events-none">
          <div className="text-[9px] text-slate-500 font-mono uppercase tracking-widest mb-1.5 flex justify-between gap-4">
            ...
            <span className="text-[#0ea5e9]">v2.4</span>
          </div>
          <div className="flex flex-col gap-1.5">
            ...
            <div className="text-[10px] font-bold text-slate-300 flex items-center pl-1">
                ...
                โซนเฝ้าระวัง 13 หมู่บ้าน (Tap to select)
            </div>
          </div>
        </div>
      </div>
