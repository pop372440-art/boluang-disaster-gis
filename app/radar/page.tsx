...
// ----------------------------------------
// 🗺️ Component จัดการการคลิกบนแผนที่
// ----------------------------------------
...
export default function RadarPage() {
  ...
  // 📍 State สำหรับระบบ Interactive Popup (ONWR Logic)
  ...
  // 🗺️ เลือก Basemap จาก Google Maps (ปลอดภัย 100% ไม่มีกล่อง Error)
  const getBasemapUrl = () => {
    switch (mapStyle) {
  ...
  // 📡 การทำงานเมื่อผู้ใช้คลิกบนแผนที่ (ONWR Architecture)
  const handleMapClick = async (lat: number, lng: number) => {
    ...
    try {
      // ดึงข้อมูลพยากรณ์รายชั่วโมงจาก Open-Meteo
      ...
      // หาเวลาปัจจุบันเพื่อดึงข้อมูล 3 ชั่วโมงข้างหน้า
      const now = new Date();
      const currentHourStr = now.toISOString().slice(0, 13) + ":00"; 
      const hourlyTimes = data.hourly.time;
      let currentIndex = hourlyTimes.findIndex((t: string) => t.includes(currentHourStr));
      
      if (currentIndex === -1) currentIndex = 0; // fallback

      // ดึงข้อมูล +1, +2, +3 ชั่วโมง
      const next3Hours = [
        { 
          time: new Date(hourlyTimes[currentIndex + 1]), 
          rain: data.hourly.precipitation[currentIndex + 1], 
          code: data.hourly.weathercode[currentIndex + 1] 
        },
        { 
          time: new Date(hourlyTimes[currentIndex + 2]), 
          rain: data.hourly.precipitation[currentIndex + 2], 
          code: data.hourly.weathercode[currentIndex + 2] 
        },
        { 
          time: new Date(hourlyTimes[currentIndex + 3]), 
          rain: data.hourly.precipitation[currentIndex + 3], 
          code: data.hourly.weathercode[currentIndex + 3] 
        }
      ];

      // คำนวณภาพรวมว่า "มีฝน" หรือ "ไม่มีฝน"
      const totalRain = next3Hours.reduce((sum, h) => sum + h.rain, 0);
      const isRaining = totalRain > 0.5;
  ...
  // สร้าง Icon หมุดสีฟ้า (เหมือนของ สทนช.)
  const L = typeof window !== 'undefined' ? require('leaflet') : null;
  const customPinIcon = L ? L.divIcon({
      ...
      <header className="h-[50px] bg-[#1A1D24] border-b border-[#2D323B] z-[1000] flex items-center justify-between px-4 shrink-0 shadow-sm">
        <div className="flex items-center space-x-3">
          ...
          <h1 className="text-[14px] font-bold tracking-wide text-[#E5E7EB]">
            เรดาร์และปริมาณฝน (Nowcast) <span className="text-[11px] text-[#4178F3] font-normal ml-2 hidden md:inline">เทศบาลตำบลบ่อหลวง จ.เชียงใหม่</span>
          </h1>
        </div>
      </header>
      ...
      <div className="relative flex-1">
        ...
        <div className="absolute inset-0 z-0">
          <MapContainer center={[center.lat, center.lng]} zoom={11} maxZoom={20} zoomControl={false} attributionControl={false} className="w-full h-full">
            ...
            {/* BaseMap ของ Google (แก้ปัญหา Zoom Level Not Supported ถาวร) */}
            <TileLayer 
              url={getBasemapUrl()} 
              maxZoom={20} 
            ...
            {/* เรดาร์ฝน (ใช้ maxNativeZoom=12 เพื่อให้รูปยืดขยายแทนการดึงรูปที่ไม่มีอยู่จริง) */}
            {radarUrl && <TileLayer key={activeFrame?.path} url={radarUrl} opacity={radarOpacity} zIndex={100} maxNativeZoom={12} maxZoom={20} />}
            
            {/* 🎯 จัดการคลิกบนแผนที่ และปักหมุด */}
            <ClickableMap onMapClick={handleMapClick} />
            {clickedLocation && customPinIcon && (
              <Marker position={[clickedLocation.lat, clickedLocation.lng]} icon={customPinIcon} />
            )}
            ...
            <div className="p-4 space-y-5">
              ...
              <div className="space-y-4">
                ...
                <label className="flex items-center space-x-3 cursor-pointer group">
                  ...
                  <span className="text-[13px] font-medium text-[#D1D5DB] group-hover:text-white">ขอบเขตตำบลบ่อหลวง</span>
                </label>

                <label className="flex items-center space-x-3 cursor-pointer group">
                  <input type="checkbox" checked={showBlock} onChange={(e) => setShowBlock(e.target.checked)} className="onwr-checkbox" />
                  <span className="text-[13px] font-medium text-[#D1D5DB] group-hover:text-white">โซนหมู่บ้าน</span>
                </label>
              </div>
            </div>
          </div>
        ...
        {/* 🌟 THE ONWR ARCHITECTURE: Interactive Modal (พยากรณ์ล่วงหน้าจุดที่คลิก) 🌟 */}
        {clickedLocation && (
          <div className="absolute top-4 right-4 z-[1050] w-[320px] onwr-panel flex flex-col pointer-events-auto animate-fade-in">
            {/* Header ของ Modal */}
            <div className="px-4 py-3 border-b border-[#333946] flex justify-between items-center bg-[#1A1D24] rounded-t-lg">
              ...
              <button onClick={() => setClickedLocation(null)} className="text-[#EF4444] hover:text-red-400 bg-red-500/10 p-1 rounded-md transition-colors">
                ...
                <>
                  ...
                  <div className={`p-3 rounded-lg border ${forecastData.isRaining ? 'bg-yellow-500/10 border-yellow-500/30' : 'bg-green-500/10 border-green-500/30'} mb-4`}>
                    <div className="flex items-center space-x-2">
                      ...
                      <h4 className={`text-[13px] font-bold ${forecastData.isRaining ? 'text-yellow-400' : 'text-green-400'}`}>
                        {forecastData.isRaining ? 'คาดว่าจะมีฝน' : 'ไม่มีฝน'} (ใน 3 ชม. ข้างหน้า)
                      </h4>
                    </div>
                    <p className="text-[11px] text-[#D1D5DB] mt-1.5 ml-4">
                      {forecastData.isRaining ? 'สภาพอากาศแปรปรวน ท้องฟ้าครึ้ม' : 'สภาพอากาศปกติ ท้องฟ้าโปร่ง'}
                    </p>
                  </div>

                  {/* กล่องพยากรณ์รายชั่วโมง 3 กล่อง */}
                  ...
                  {/* สรุปข้อมูลพื้นที่ */}
                  <h5 className="text-[11px] font-bold text-[#8B94A5] mb-2 border-t border-[#333946] pt-3">ภาพรวมพื้นที่ 3 ชม. ข้างหน้า</h5>
                  <div className="space-y-2 bg-[#1A1D24] border border-[#333946] rounded-lg p-3">
                    <div className="flex justify-between items-center text-[12px]">
                      <span className="text-[#8B94A5]">พื้นที่ <span className="text-[#E5E7EB] font-bold">ต.บ่อหลวง</span></span>
                      <span className="text-gray-400 font-bold bg-[#333946] px-2 py-0.5 rounded text-[10px]">ตรวจสอบจากพิกัด</span>
                    </div>
                  </div>

                  <button onClick={() => setClickedLocation(null)} className="w-full mt-4 py-2.5 bg-[#4178F3] hover:bg-blue-600 text-white rounded-lg text-[13px] font-bold transition-colors">
                    ปิดหน้าต่าง
                  </button>
                </>
              ) : (
        ...
        {/* 📊 Legend แถบสี */}
        <div className="absolute bottom-[100px] md:bottom-6 left-4 z-[900] pointer-events-auto">
