...
export default function ExecutiveDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [onwrRes, forecastRes, deepmindRes] = await Promise.allSettled([
          ...
          // 📡 เพิ่ม current=precipitation เพื่อดึง "ฝนตก ณ วินาทีนี้"
        ...
        if (forecast && deepmindForecast) {
            ...
            const liveRainIntensity = forecast.current.precipitation || 0; // ฝนรายชั่วโมง ณ ตอนนี้
            const maxRain7Days = Math.max(...forecast.daily.precipitation_sum);
            
            // ⛰️ คำนวณดัชนีดินอุ้มน้ำ (Soil Saturation) แบบง่ายๆ: ยิ่งตกต่อเนื่อง ค่าจะยิ่งสูง
            // ผสมระหว่างฝนสะสม + ฝนที่กำลังตกอยู่ ณ ตอนนี้
            ...
            // 🚨 อัปเกรด AI Logic: ถนแม้เซนเซอร์ ONWR จะขึ้นแค่ 6 มม. แต่ถ้าฝนกำลังตก (Live > 0) และดินอุ้มน้ำเกิน 50% ให้ยกระดับทันที!
            if (actualRain24h > 90 || maxRain7Days > 90 || soilMoisture > 80) {
                ...
                tmdInsight = liveRainIntensity > 0
                    ...
                    : `เฝ้าระวังพายุฝนฟ้าคะนองรุนแรงใน 1-3 วันนี้`;
    ...
    fetchData();
    // 🔄 ให้ดึงข้อมูลใหม่ทุกๆ 5 นาที (เหมาะสำหรับ Command Center ช่วงฝนตก)
  ...
  if (isLoading) return (
    <div className="flex h-screen items-center justify-center bg-[#0a1112] text-white">
        <div className="w-16 h-16 border-4 border-[#2dd4bf] border-t-transparent rounded-full animate-spin shadow-[0_0_15px_#2dd4bf]"></div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#0a1112] p-4 md:p-8 font-sans text-gray-100 flex flex-col">
        ...
        <div>
            <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight text-white flex items-center">
                ...
                {data.liveRainIntensity > 0 && (
                    <span className="ml-4 px-3 py-1 bg-red-600/20 border border-red-500 text-red-500 text-sm font-bold rounded-full animate-pulse flex items-center">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 flex-1">
        <div className="xl:col-span-5 flex flex-col gap-6">
            <div className={`flex-1 border ${theme.border} bg-[#111a1c] ${theme.glow} rounded-3xl p-8 flex flex-col transition-all duration-500`}>
                <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-800">
                    ...
                    {/* ⛰️ หลอดเกจวัดความเสี่ยงดินถล่ม (Soil Moisture/Saturation) */}
                    <div className="text-right w-32">
                        <div className="text-[10px] text-gray-400 mb-1 uppercase tracking-wider">ดัชนีดินอุ้มน้ำ (เสี่ยงถล่ม)</div>
                        <div className="w-full bg-gray-800 rounded-full h-2">
                            <div className={`${soilColor} h-2 rounded-full transition-all duration-1000`} style={{ width: `${data.soilMoisture}%` }}></div>
                        </div>
                ...
                <div className="space-y-6 flex-1">
                    <div className="p-1">
                        <h3 className="text-[#2dd4bf] font-bold text-sm mb-3 flex items-center"><span className="text-lg mr-3">🇹🇭</span> ประกาศกรมอุตุฯ & สภาพจริง</h3>
                        <p className="text-gray-300 text-base leading-relaxed pl-7 border-l-2 border-[#2dd4bf]/30">{data.ai.tmdInsight}</p>
                    </div>
                    <div className="p-1">
                        <h3 className="text-[#2dd4bf] font-bold text-sm mb-3 flex items-center"><span className="text-lg mr-3">🌎</span> พยากรณ์มหาภาค 15 วัน (DeepMind)</h3>
                        <p className="text-gray-300 text-base leading-relaxed pl-7 border-l-2 border-[#2dd4bf]/30">{data.ai.deepmindTrend}</p>
                    </div>
                </div>
            </div>

            <div className={`border ${theme.border} bg-[#111a1c] rounded-3xl p-8 shadow-lg`}>
                <h3 className="text-base text-white font-bold mb-6 flex items-center tracking-wide">
                    <span className="text-xl mr-3">🎯</span> ข้อเสนอแนะการสั่งการเชิงรุก
                </h3>
                <ul className="space-y-3">
                    {data.ai.actions.map((action: string, idx: number) => (
                        <li key={idx} className="flex items-center bg-[#0a1112] p-4 rounded-xl border border-gray-800/60">
                            <div className={`w-2 h-2 rounded-full ${theme.bg} mr-4 flex-shrink-0`}></div>
        ...
        <div className="xl:col-span-7 flex flex-col gap-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                {/* 🔴 แทนที่กล่องแรกด้วย Live Rain Intensity แทนอุณหภูมิ */}
                <div className={`bg-[#111a1c] border ${data.liveRainIntensity > 0 ? 'border-red-500/50 shadow-[0_0_15px_rgba(239,68,68,0.15)]' : 'border-gray-800'} rounded-2xl p-6 relative overflow-hidden transition-all duration-500`}>
