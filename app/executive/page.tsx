...
// ==========================================
// 🌀 2. Mock Storm Data (ทดแทน API ที่ไม่มีอยู่จริง)
// ==========================================
...
const getMockStorm = (): StormInfo | null => {
  // สร้างพายุจำลองที่กำลังเคลื่อนที่ในทะเลจีนใต้
  const pts: TrackPoint[] = [
    ...
    { lat: 16.8, lon: 105.0, time: new Date(Date.now() + 86400000).toISOString(), wind: 65, gust: 85, pressure: 990 }, // ใกล้ที่สุด
...
export default function ExecutiveDashboard() {
  ...
  // สถานะ API (เขียวหมดถ้าดึงข้อมูลหลักสำเร็จ)
  const [apiHealth, setApiHealth] = useState({ onwr: 'LOAD', tmd: 'LOAD', deepmind: 'LOAD', storm: 'MOCK', tmdStorm: 'OFFLINE' });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // 📡 ETL: Ground Truth + Deterministic
        ...
        // เปลี่ยนใช้ Endpoint ที่เสถียรสำหรับ Ensemble
        ...
        // 🌀 Storm Track (ใช้ Mock Data)
        const stormTop = getMockStorm();
        const stormInfos = [stormTop];

        setApiHealth({ onwr: onwrRes.status, tmd: forecastRes.status, deepmind: ensRes.status, storm: 'MOCK', tmdStorm: 'OFFLINE' });
        ...
        if (forecast && ens) {
          ...
          const stats = Array.from({ length: N }, (_, d) => {
            ...
            return {
              date: daily.time[d],
              rainMedian: rains.length ? median(rains) : (daily.precipitation_sum?.[d] || 0),
              rainMin: rains.length ? Math.min(...rains) : 0, 
              rainMax: rains.length ? Math.max(...rains) : (daily.precipitation_sum?.[d] || 0),
          ...
          if (groundOverride) {
            ...
            aiInsight = `AI ประเมินฝนระลอกกิจกรรมสูง พีควันที่ ${fmtDate(peakRainDay.date)} (P(ฝน≥50มม.) ${Math.round(peakRainDay.pRain50)}%)`;
  ...
  // ============ 🎨 UI ============
  if (isLoading) return (
    <div className="flex h-screen items-center justify-center bg-[#070b0f] text-white">
      <div className="flex flex-col items-center">
        ...
        <span className="font-mono text-[#0ea5e9] text-xl tracking-[0.2em] animate-pulse font-bold">INITIALIZING CORE SYSTEM...</span>
      </div>
    </div>
  );
  if (!data) return (
    ...
    <div className="min-h-screen bg-[#020617] p-4 md:p-8 font-sans text-slate-200 overflow-x-hidden selection:bg-[#0ea5e9] selection:text-white">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-8 pb-6 border-b border-slate-800/80 gap-6">
        <div>
          ...
          <p className="text-[#0ea5e9] text-xs sm:text-sm md:text-base tracking-[0.15em] font-mono font-medium">INTELLIGENCE PLATFORM FOR HAZARD ANALYTICS</p>
        </div>
        <div className="flex flex-col items-start lg:items-end w-full lg:w-auto">
          ...
          <div className="flex flex-wrap gap-2 w-full lg:justify-end">
            ...
            <HealthBadge label="OPEN-METEO (SAT)" status={apiHealth.tmd} />
            <HealthBadge label="ICON-SEAMLESS (AI)" status={apiHealth.deepmind} />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 md:gap-8 flex-1 z-10">
        ...
        <div className="xl:col-span-5 flex flex-col gap-6 md:gap-8 h-full">
            <div className={`flex-1 border ${theme.border} bg-[#0b1120] ${theme.glow} rounded-[2rem] p-6 md:p-8 flex flex-col transition-all duration-500`}>
                <div className="flex items-center space-x-4 mb-6 pb-5 border-b border-slate-800">
                    ...
                    <div>
                        <h2 className={`text-xl md:text-3xl font-black tracking-tight ${theme.text}`}>Data Fusion Engine</h2>
        ...
        <div className="xl:col-span-7 flex flex-col gap-6 md:gap-8">
            ...
            {/* 🔮 15-Day Chart (SVG แก้บั๊กแล้ว) */}
            <div className="bg-[#0b1120] border border-slate-800 rounded-[2rem] p-6 md:p-8 shadow-xl flex-1 flex flex-col min-h-[350px]">
              <div className="mb-6">
                <h3 className="text-xl md:text-2xl font-black text-white flex items-center tracking-tight"><span className="text-3xl mr-3">🔮</span> 15-DAY ENSEMBLE FORECAST</h3>
                <p className="text-xs text-slate-500 font-mono mt-1 tracking-widest">ICON-SEAMLESS MODEL • PROBABILITY TIER</p>
              </div>
              
              <div className="flex-1 w-full h-full relative">
                {/* SVG Graph 15 วัน - เรนเดอร์ถูกต้อง 100% */}
