...
export default function StatusPage() {
  ...
  const getStatusColor = (status: string) => {
    ...
    return 'bg-blue-100 text-blue-700 border-blue-200'; // รับเรื่องแล้ว
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center py-10 px-4 font-sans">
      ...
      <div className="w-full max-w-lg mb-8 flex flex-col items-center">
        <div className="w-16 h-16 bg-blue-600 rounded-full flex items-center justify-center shadow-lg mb-4 text-2xl">
          🔍
        </div>
      ...
      {reportData && (
        <div className="w-full max-w-lg bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden animate-fade-in-up">
          <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
            ...
            <span className="font-mono text-sm font-bold text-gray-800 bg-white px-3 py-1 rounded-lg border shadow-sm">
              {reportData.tracking_code}
            </span>
          </div>
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between border-b pb-2">
                <span className="text-gray-500 font-medium">ประเภทภัย:</span>
                <span className="font-bold text-gray-800">{reportData.risk_type}</span>
              </div>
              <div className="flex justify-between border-b pb-2">
                <span className="text-gray-500 font-medium">พื้นที่เกิดเหตุ:</span>
                <span className="font-bold text-gray-800">{reportData.village_name}</span>
              </div>
              <div className="flex justify-between border-b pb-2">
                <span className="text-gray-500 font-medium">ระดับความรุนแรง:</span>
                <span className="font-bold text-red-600">ระดับ {reportData.severity_level}</span>
              </div>
              <div className="flex flex-col border-b pb-2">
                <span className="text-gray-500 font-medium mb-1">รายละเอียด:</span>
                <span className="text-gray-700 bg-gray-50 p-3 rounded-lg border">{reportData.description}</span>
              </div>
              <div className="flex justify-between border-b pb-2">
                <span className="text-gray-500 font-medium">วันที่แจ้ง:</span>
                <span className="text-gray-700">{new Date(reportData.created_at).toLocaleString('th-TH')}</span>
              </div>
            </div>
